...
| SAP 구축·운영 이해 | DEV/QAS/PRD, IMG, 권한, Transport, UAT, Cutover, Incident/Change, Migration 산출물 |
...
이 프로젝트는 실제 고객사 또는 사내 SAP 운영 시스템을 사용한 경험으로 포장하지 않습니다. 대신 실제 SAP ERP/S/4HANA 구축·운영 프로젝트에서 요구되는 항목을 개인 프로젝트의 산출물과 점검 로직으로 확장했습니다.

| 실제 구축·운영 항목 | 프로젝트에서 확장한 내용 |
| --- | --- |
| 고객사/사내 SAP ERP 또는 S/4HANA 시스템 사용 | 실제 연동 시 교체해야 할 BTP Destination, SAP API, 권한, Transport 지점 정의 |
| FI/CO, MM, SD, PP, SCM 모듈 설정 또는 개발 | 협력사 기준정보를 MM/SCM 중심으로 두고 FI 지급조건, 조정계정, 세금번호와 연결해 Fit-Gap 작성 |
| 운영 테넌트 및 개발·검증·운영 landscape | DEV/QAS/PRD 기준의 개발, 검증, 운영, Hypercare 흐름 정의 |
| IMG 설정, 권한, Transport, 배포, 테스트, 장애 대응 | IMG 설정 후보, role 설계, Transport 전략, UAT, Cutover, 운영 Runbook 작성 |
| 실사용자 요구사항, 변경관리, 운영 이슈 처리 | 요구사항 backlog, Incident Register, Change Request Register 템플릿 추가 |
| 실제 ERP 데이터 생성·변경·마이그레이션 | Supplier master migration 샘플, 필수값 검증, source/target reconciliation 로직 추가 |

확장 산출물은 `implementation-ops/`에 정리되어 있습니다. `npm run ops:check`로 샘플 마이그레이션 데이터와 Go-Live readiness를 점검할 수 있습니다.
...
- ABAP RAP 승인 규칙 산출물
- SAP ERP 구축·운영 산출물 패키지
- 마이그레이션 필수값 검증 및 Go-Live readiness 체크
...
docs/                아키텍처와 데모 스크립트
implementation-ops/  ERP 구축·운영 연계 산출물과 운영 템플릿
scripts/             운영 readiness 점검 스크립트
...
- SAP ERP 구축·운영에서 필요한 landscape, Transport, 권한, UAT, Cutover 이해
- 마이그레이션 데이터 검증과 운영 대사 관점
- 실제 구현 범위와 모의 구현 범위를 구분하는 정직한 개발 태도

## Summary

SAP BTP에서 CAP(Node.js)·Fiori 기반 협력사 등록/승인 시스템과 ABAP Cloud RAP 승인 규칙 앱 산출물을 설계했습니다. S/4HANA Business Partner API 조회 Adapter, Mock ERP 쓰기, 상태 전이, 멱등성, 실패 재처리, Integration Log, CDS/Behavior/OData V4/ABAP Unit 흐름을 다뤘습니다. 추가로 실제 SAP ERP 구축·운영 프로젝트에서 필요한 DEV/QAS/PRD, IMG, 권한, Transport, UAT, Cutover, Incident/Change, Migration 산출물까지 연결했습니다.
